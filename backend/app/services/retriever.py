...
"""
ENHANCED RETRIEVER - Phase 2.5 + Gemini Improvements
- Query Expansion (3 variations for better coverage)
- Re-Ranking (LLM grades relevance)
- Hybrid Search (keyword + semantic if available)
"""
import asyncio
import random
import logging
...
class EnhancedRetriever:
    """
    PRODUCTION HYBRID with Gemini improvements:
    ...
    # ========== GEMINI IMPROVEMENT: QUERY EXPANSION ==========
    
    async def _generate_query_variations(self, question: str) -> List[str]:
        """
        GEMINI IMPROVEMENT: Generate 3 query variations
...
Queries:"""
    ...
    # ========== GEMINI IMPROVEMENT: RE-RANKING ==========
    
    async def _rerank_nodes(self, question: str, nodes: List[Dict]) -> List[Dict]:
        """
        GEMINI IMPROVEMENT: LLM grades relevance
